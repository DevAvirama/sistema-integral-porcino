        ...
        <div className="space-y-8">
          ...
          <div className="space-y-5">
            ...
            <p className="max-w-2xl text-lg leading-8 text-slate-600">
              Esta nueva base traslada el proyecto original a una aplicacion React con
              Tailwind, dejando lista una experiencia inicial con landing, inicio de
              sesion y dashboard.
            </p>
          </div>
          <div className="flex flex-wrap gap-4">
            <Button as={Link} to="/dashboard" tone="accent" className="shadow-lg shadow-emerald-500/30">
              Ver dashboard
            </Button>
            <Button as={Link} to="/login" tone="secondary">
              Iniciar sesion
            </Button>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-white/60 bg-slate-950 p-6 shadow-2xl shadow-slate-950/20">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Card className="rounded-3xl bg-white/5 p-5 text-white ring-1 ring-white/10 shadow-none">
              ...
              <Card className="rounded-3xl p-5 text-slate-950 shadow-none">
                <p className="text-sm text-slate-500">Alertas activas</p>
                <p className="mt-2 text-4xl font-black">06</p>
                <p className="mt-4 text-sm text-slate-600">Dos criticas y cuatro preventivas.</p>
              </Card>
              <Card className="rounded-3xl bg-emerald-400 p-5 text-slate-950 shadow-none sm:col-span-2">
                <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                  ...
                  <p className="max-w-sm text-sm leading-6 text-emerald-950/80">
                    La estructura base ya esta pensada para migrar progresivamente el resto
                    del sistema.
                  </p>
                </div>
              </Card>
      ...
      <section id="impacto" className="border-y border-slate-200/70 bg-white/70">
        <div className="mx-auto grid max-w-7xl gap-6 px-6 py-10 sm:grid-cols-2 lg:grid-cols-4 lg:px-10">
          {landingStats.map((stat) => (
            <Card
              key={stat.label}
              className="rounded-3xl border border-slate-200 px-6 py-5 shadow-sm"
            >
      ...
      <section id="caracteristicas" className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {landingFeatures.map((feature, index) => (
            <Card key={feature.title} className="rounded-[2rem] border border-slate-200 p-7">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-20 lg:grid-cols-[0.85fr_1.15fr] lg:px-10">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {['Landing publica', 'Inicio de sesion', 'Dashboard general'].map((item) => (
              <Card
                key={item}
                className="rounded-[1.75rem] border border-white/10 bg-white/5 p-5 text-slate-200 shadow-none"
              >
